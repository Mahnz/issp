...
class Server(FileServer):
    ...
    def authorize(self, user: str, file: str, action: str) -> bool:
        acl = self.permissions.get(file, {})  # Default to an empty ACL.
        permissions = acl.get(user, set())  # Default to an empty set of permissions.
    ...
    def change_permissions(self, msg: JSONMessage) -> JSONMessage:
        permissions = set(msg["permissions"])
        permissions &= {"read", "write"}  # Remove disallowed permissions.
        self.permissions[msg["path"]][msg["target"]] = permissions
